...
const MessageBubble = ({ message }: MessageBubbleProps) => {
  ...
  return (
    <div className={`flex gap-4 p-4 group ${isUser ? 'bg-transparent' : 'bg-scraper-bg-card/30'}`}>
      {/* Avatar */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        isUser 
          ? 'bg-scraper-gradient-primary shadow-scraper-glow' 
          : 'bg-scraper-bg-card border border-scraper-border'
      }`}>
        {isUser ? (
